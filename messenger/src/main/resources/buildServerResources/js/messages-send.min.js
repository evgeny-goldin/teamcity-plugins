...
d=ms.recipientsValidate();c&&b&&d&&(a("#messages-send-button").disable(),a("#messages-send-progress").show(),a.ajax({url:a(this).attr("action"),data:a(this).serialize(),type:"POST",dataType:"text",success:function(b){ms.dialog('Message "'+b+'" sent',1);a("#messages-send-text").val("").change()},error:function(){ms.dialog("Failed to send message",-1)},complete:function(){a("#messages-send-progress").hide()}}));return!1}};a(function(){a("#messages-send-counter").text(dialog_const.text_max_length);a("#messages-send-longevity-number").change(ms.longevityValidate);
a("#messages-send-text").keyup(ms.textValidate).change(ms.textValidate).focus();a("#messages-send-all, #messages-send-groups, #messages-send-users").change(ms.recipientsValidate);a("#messages-send-form").submit(ms.formSubmit);a("#messages-send-dialog-ok").click(function(){ms.dialogClose();return!1});a("#messages-send-all").click().change()})})(jQuery.noConflict());