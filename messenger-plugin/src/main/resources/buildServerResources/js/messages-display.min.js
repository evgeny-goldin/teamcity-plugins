...
b);md.dialog({title:md.titleTemplate.evaluate(a),text:a.text,urgency:a.urgency})},getMessages:function(){j.get(md.action,{t:j.now()},function(a){if(a&&a.length){if(md.nMessages()<1||md.nMessages()!=a.length||a.any(md.unknownMessage))md.messageDisplayed=0,md.messages=a.slice(0),j.assert(md.nMessages(),"getMessages(): ["+md.nMessages()+"] (md.nMessages())"),md.dialogMessage()}else md.dialogClose()},"json")},dialogClose:function(){j("#messages-display-dialog").dialog("destroy")}};
