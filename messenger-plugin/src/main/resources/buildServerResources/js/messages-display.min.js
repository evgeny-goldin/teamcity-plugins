...
"] (md.messageDisplayed)");var a;for(a=md.messageDisplayed-1;md.messages[a].deleted;a--);j.assert(a>-1&&a<md.messageDisplayed,'"Prev" click: ['+a+"]["+md.messageDisplayed+"] (prevMessage, md.messageDisplayed)");md.messageDisplayed=a;md.dialogMessage()},nextMessage:function(){j.assert(md.messageDisplayed<md.nMessages()-1,'"Next" click: ['+md.messageDisplayed+"]["+md.nMessages()+"] (md.messageDisplayed, md.nMessages())");var a;for(a=md.messageDisplayed+1;md.messages[a].deleted;a++);j.assert(a<md.nMessages()&&
a>md.messageDisplayed,'"Next" click: ['+a+"]["+md.messageDisplayed+"] (nextMessage, md.messageDisplayed)");md.messageDisplayed=a;md.dialogMessage()},dialogClose:function(){j("#messages-display-dialog").dialog("destroy");return!1},deleteMessage:function(){j("#messages-display-progress").show();j.assert(md.messageDisplayed>-1&&md.messageDisplayed<md.nMessages(),'"Delete" click: ['+md.messageDisplayed+"] (md.messageDisplayed)");var a=md.messages[md.messageDisplayed];j.ajax({url:md.action,type:"POST",
data:{id:a.id},dataType:"text",success:function(b){j.assert(a.id==b,'"Delete" click: ['+a.id+"] != ["+b+"]");a.deleted=!0;md.dialog({title:"Message Deleted",text:'Message "'+b+'" deleted',statusMessage:!0});(function(){var a=md.messages.newIndexOf(md.isDeleted,!0,md.messageDisplayed+1);a<0&&(a=md.messages.newIndexOf(md.isDeleted,!0,0,md.messageDisplayed));a<0?md.dialogClose():(md.messageDisplayed=a,md.dialogMessage())}).delay(1)},error:function(){md.dialog({title:"Message not Deleted",text:'Failed to delete message "'+
messageId+'"',statusMessage:!0,urgency:"critical"})},complete:function(){j("#messages-display-progress").hide()}});return!1}};j(function(){j("#messages-display-dialog-prev").click(md.prevMessage);j("#messages-display-dialog-next").click(md.nextMessage);j("#messages-display-dialog-close").click(md.dialogClose);j("#messages-display-dialog-delete").click(md.deleteMessage)});